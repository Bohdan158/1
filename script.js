    ...
    for (let row = 0; row < gridSize; row++) {
        ...
        for (let col = 0; col < gridSize; col++) {
            gridRow.push({
                ...
                completed: false, // If the box is completed
            });
        }
        grid.push(gridRow);
    }
    ...
    for (let row = 0; row < gridSize; row++) {
        for (let col = 0; col < gridSize; col++) {
            ...
            if (grid[row][col].completed) {
                ctx.fillStyle = grid[row][col].owner === 'player' ? 'rgba(0, 0, 255, 0.5)' : 'rgba(255, 0, 0, 0.5)';
                ctx.fillRect(col * dotSpacing + 51, row * dotSpacing + 51, dotSpacing - 2, dotSpacing - 2);
            }
        }
    }
}
    ...
    for (let row = 0; row < gridSize; row++) {
        for (let col = 0; col < gridSize; col++) {
            ...
            if (!grid[row][col].right && Math.abs(y - dotY) < clickMargin && x > dotX && x < dotX + dotSpacing) {
                ...
                const boxCompleted = checkForBox(row, col);
                drawGrid();
...
function checkForBox(row, col) {
    ...
    if (row < gridSize - 1 && col < gridSize - 1) {
        if (
            ...
            grid[row][col + 1].down &&
            !grid[row][col].completed
        ) {
            grid[row][col].owner = currentPlayer;
            grid[row][col].completed = true;
            boxCompleted = true;
    ...
    // Check box above (if applicable)
    if (row > 0 && !grid[row - 1][col].completed) {
        if (
            grid[row - 1][col].right &&
            grid[row - 1][col].down &&
            grid[row][col].right &&
            grid[row - 1][col + 1].down
        ) {
            grid[row - 1][col].owner = currentPlayer;
            grid[row - 1][col].completed = true;
            boxCompleted = true;
            if (currentPlayer === 'player') playerScore++;
            else aiScore++;
        }
